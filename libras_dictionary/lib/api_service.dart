...
class ApiService {
  ...
  Future<List<DictItem>> search(String query, {String source = 'Ambos'}) async {
    ...
    if (source == 'Ambos' || source == 'LibrasAcademicaUFF') {
      futures.add(
        _fetchLibrasAcademicaUFF(query).catchError((_) => <DictItem>[]),
      );
    }
    ...
    if (response.statusCode == 200) {
      ...
      final filteredData = data.where((item) {
        ...
        final normalizedTitle = title != null
            ...
            : '';
        final normalizedContent = content != null
            ? removeDiacritics(content).toLowerCase()
            : '';
  ...
  Future<List<DictItem>> _fetchInes(String query) async {
    ...
    if (_cachedInesData != null) {
      ...
      for (var item in _cachedInesData!) {
        ...
        final normalizedPalavra = palavra != null
            ? removeDiacritics(palavra).toLowerCase()
            : '';
        final normalizedDescricao = descricao != null
            ? removeDiacritics(descricao).toLowerCase()
            : '';
      ...
      for (final item in data) {
        ...
        if (title != null && content != null) {
          ...
          if (wordBound.hasMatch(normalizedTitle) ||
            ...
            if (videoUrl == null && youtubeId == null) {
              final RegExp ytShortExp = RegExp(
                r'https:\/\/youtu\.be\/([^"&\s<]+)',
              );
